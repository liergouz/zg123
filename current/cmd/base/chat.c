...
/*      if( me->is_die() )
	...
	"/quest/help"->send_help_tips(me, 54);
        ...
        if (!is_god(me)) 
        {
        	...
        	legend = sprintf("聊天 %s,%d,%s,%s @%s 金 0 %d 0 %d_%d_%d", me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), "公众", 200, get_z(me), get_x(me), get_y(me) );
