...
int open_box(object me)
{
	...
	if (item->is_cash())
	{
		i = item->get_value();
		me->add_cash(i);
		call_out("message_box", 1, me, 1, sprintf("%d金", i));		
...
{
	send_user(me, "%c%s", ';', sprintf("恭喜您获得了"HIR" %s "NOR"。", name));
	if (type==2) USER_D->user_channel( sprintf("恭喜%s玩家开启宝箱获得了"HIR" %s "NOR"的奖励，让我们对其表示祝贺！", me->get_name(), name));
}
