...
int main(object me,string arg) {
	...
	switch(cmds[0]) {
		case "open":
			if( !objectp(item = present("战国宝典", me, 1, MAX_CARRY*4)) ) {
				return 0;
			}
...
void send_gala_message(object player,string item_name) {
	if(objectp(player) && item_name) {
		send_user(player,"%c%s",';',sprintf("恭喜你获得了"HIR"%s"NOR"！",item_name));
	}	
}
...
void zgdict_rock_item(object player) {
	...
	if(item->is_cash()) {
		i = item->get_value();
		player->add_cash(i);		
		user_get_item += sprintf(HIR"%d"NOR"金 ",i);
