...
//                                        ESC "丢弃。\n" + sprintf( "drop ! %x#%d\n", getoid(item), amount ) +
//                                        ESC "我再想想……\nCLOSE\n" );
                        send_user( me, "%c%c%s", 0x59, 1, sprintf(item->get_name() + "被丢弃后将会消失。您确定要丢弃吗？\n"
                        	ESC "%c\n"ESC"drop ! %x#%d\n"
//                                ESC "%c\n"ESC"team cancel\n",
                ...
                else    // 垃圾消失
                {
                	if (item->is_record())
                	{
                		result = sprintf("丢弃 %s,%d,%s,%s @%s %s %s %d %s %d_%d_%d", me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), "", item->get_name(), "/sys/item/item"->get_pawn_save_string(item), amount, item->get_user_id(), get_z(me), get_x(me), get_y(me) );
				"/sys/user/cmd"->log_item(result);
                	}
                        if( !( total = item->get_amount() ) )
                        {
                                printf( ECHO "您丢下%s，%s随即消失了。", item->get_name(), item->get_name() );
