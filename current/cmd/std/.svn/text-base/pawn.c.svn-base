...
/*              if( item->is_locked() )
                ...
                {
                        notify( item->get_name() + "已经上架，不能典当。" );
                        send_user( me, "%c%d%c", 0x4e, getoid(item), item->get_amount() );    // 补发消息
                        return 1;
                }
...
/*              for( i = 0; i < size; i ++ ) if( to_int( key[i] ) != i + 1 ) break;
                ...
                &&      sscanf( name, "%s;%d;%s", file, total, prop ) && load_object(file) )
                {
                	if ( (file==get_file_name(item)) && item->get_max_combined()>1 )
                	{
                		...
                		if (item->is_record())
                		{
                			legend = sprintf("存入 @%s %s,%d,%s,%s %s %s %d %s %d_%d_%d", who->get_name(), me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), item->get_name(), "/sys/item/item"->get_pawn_save_string(item), amount, item->get_user_id(), get_z(me), get_x(me), get_y(me) );
...
//tell_user(me, prop);		
                if( item->get_amount() )
                        result = sprintf( "您把 %s存入%s处。", item->get_name(), who->get_name() );
                else    result = sprintf( "您把%s存入%s处。", item->get_name(), who->get_name() );
                notify(result);
...
//              if( amount < 1 ) return 1;
	...
	case ITEM_TYPE_DIAMOND_3:
                ...
                if( item->get_amount() )
                        result = sprintf( "您从%s处取回 %s。", who->get_name(), item->get_name() );
                else    result = sprintf( "您从%s处取回 %s。", who->get_name(), item->get_name() );
                notify(result);
                write_user(me, ECHO + result);
		if (item->is_record())
                {
...
int expand_pawn( object me, string name,int flag)
{
	...
	if ( flag <= 0 )
	{
		send_user( me, "%c%c%w%s", ':', 3, who->get_char_picid(),sprintf("%s\n    你现在拥有%d个当铺空间，如果要增加新的空间，就必须花费%d金。你确定要开启新的空间吗？\n"ESC"确定\npawn expand %x# 1\n"ESC"离开",who->get_name(),size,cash,getoid(who)));
		return 1;
	}
