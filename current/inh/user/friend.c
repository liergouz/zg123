...
//              send_user( me, "%c%c%d", 0x46, 0, who->get_number() );
        ...
        {
        	if (me->get_gender()==1)
        		note->Name = sprintf("%s(娘子)", who->get_name());
        	else
        		note->Name = sprintf("%s(相公)", who->get_name());
        }
        else
        if (who->get_save_2("baishi.tudi.id")==me->get_number())
        	note->Name = sprintf("%s(师傅)", who->get_name());
	else
        if (who->get_save_2("baishi.shifu.id")==me->get_number())
        	note->Name = sprintf("%s(徒弟)", who->get_name());	        	
        note->Type = type;
//      note->Notify = 0;
        ...
        {
  case "齐国" : note->City = 1;  break;
  case "韩国" : note->City = 2;  break;
  case "赵国" : note->City = 3;  break;
  case "魏国" : note->City = 4;  break;
  case "秦国" : note->City = 5;  break;
  case "楚国" : note->City = 6;  break;
  case "燕国" : note->City = 7;  break;
	...
	result = sprintf("%-6s%-10s%s", note->Family, note->Org, note->Name );
        ...
        if( flag ) 
        {
        	...
        	who->delete_friend_degree(me->get_number());
                write_user( who, ECHO "%s(%d)把您添加为好友。", me->get_name(), me->get_number() );
                send_user( who, "%c%s", '!', sprintf( HIY "%s(%d) " NOR "把您添加为好友。", me->get_name(), me->get_number() ) );
        }
}
        ...
        else for( i = 0; i < size; i ++ )
        {
                if( ( (class Friend) Friends[ key[i] ] ) ->Notify && objectp( who = find_player( key[i] ) ) ) 
                {
                        ...
                        send_user( who, "%c%c%d%s", 0x4d, 0xff, me->get_number(), 
                                sprintf( HIY "%s(%d) " NOR "上线了。", me->get_name(), me->get_number() ) );
                }
        }
}
...
void receive_friends()
{
	...
	result = sprintf("%-6s%-10s%s", "", "", "万事通" );
        send_user( me, "%c%c%d%c%c%c%c%w%s", 0x46, 1, 0, 0, 0, 2, 8, 100, result );
	result2 = sprintf("您的好友%s(%d)上线了。", me->get_name(), id);
        for( i = 0; i < size; i ++ )
        {
                note = Friends[ key[i] ];
...
void leave_friends()
{
	...
	result = sprintf("您的好友%s离线了。", me->get_name());
        for( i = 0; i < size; i ++ )
        {
                note = Friends[ key[i] ];
                if( objectp( who = find_player( key[i] ) )  ) 
