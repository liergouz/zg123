...
varargs int set_exp( int exp, int bil_exp )
{
        ...
        if( Level < 10 && UpgradeExp >= USER_EXP_D->get_level_exp(Reincarnation, Level) )
        {
	        ...
	        USER_ENERGY_D->count_all_prop(me);
                ...
                result = sprintf( HIC "您升至 %d 级！", Level );
...
{
        if( point > get_max_potential() ) send_user( this_object(), "%c%s", '!', "您的潜能已满，不能再增加了。" );
                                ...
                                if( leader = me->get_bonus_leader() )
                                {
                                  ...
                                  case 20..29 : point = (level - MaxLevel) * 40;  break;
                                      default : point = 0;  break;
                                        ...
                                        write_user(leader, ECHO "您帮助" HIY "%s" ECHO "升到 %d 级(%+d)，您功德 " HIY "%+d.%d" ECHO "。",
...
{
        if( gold > get_max_cash() ) send_user( this_object(), "%c%s", '!', "您的现金已达上限，不能再增加了。" );
...
int add_cash( int gold )
{
	...
	size = sizeof(all);
        ...
        {
        	if (gold>0)
        		result = sprintf("获得 @%s %s,%d,%s,%s 金 0 %d 0 %d_%d_%d", obj->get_name(), who->get_name(), who->get_number(), get_ip_name(who), who->get_id(),  gold, get_z(who), get_x(who), get_y(who) );
        	else
        		result = sprintf("扣减 @%s %s,%d,%s,%s 金 0 %d 0 %d_%d_%d", obj->get_name(), who->get_name(), who->get_number(), get_ip_name(who), who->get_id(),  gold, get_z(who), get_x(who), get_y(who) );
        ...
        {
	        if (gold>0)
	        	result = sprintf("获得 @%s %s,%d,%s,%s 金 0 %d 0 %d_%d_%d", get_file_name(obj), who->get_name(), who->get_number(), get_ip_name(who), who->get_id(),  gold, get_z(who), get_x(who), get_y(who) );
	        else
	        	result = sprintf("扣减 @%s %s,%d,%s,%s 金 0 %d 0 %d_%d_%d", get_file_name(obj), who->get_name(), who->get_number(), get_ip_name(who), who->get_id(),  gold, get_z(who), get_x(who), get_y(who) );
	        "/sys/user/cmd"->log_item(result);
...
{
        if( gold > get_max_savings() ) send_user( this_object(), "%c%s", '!', "您的存款已达上限，不能再增加了。" );
...
// 函数：获取ＰＫ标志
int get_pk() { return range_value(PK, 0, 65535); }    // 警告：防止溢出情况
...
int set_pk( int flag )
{
        ...
        if (old_pk<5 && PK>=5)
        {
                send_user( me, "%c%c%w%s", ':', 3, 0, "因为你罪孽深重，护身神灵暂时离你而去。你徘徊在地狱边缘，接受战火的洗礼吧。\n"
                        ESC "确定\nCLOSE\n" );
        }
        else
        if (old_pk>0 && PK==0)
        {
                send_user( me, "%c%c%w%s", ':', 3, 0, "因为你真诚忏悔，你已经放下屠刀。你已经进入和平的境界。\n"
                        ESC "确定\nCLOSE\n" );
        }
        else
        if (PK>old_pk)
    ...
    case 3..4 : me->set_char_type(PK_TYPE_3);  break;
      default : me->set_char_type(PK_TYPE_5);  break;
        ...
        return range_value(PK, 0, 65535);    // 警告：防止溢出情况
}
