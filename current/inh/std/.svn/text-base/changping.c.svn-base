...
void heart_beat_loop_callout( object map, int effect1, int effect2, int effect3, int effect4, int effect5, int effect6, int effect7, int effect8 )    // 每１分处理
{
        ...
        zhao = map->get("zhao1");
        if ( !qin || !zhao )	//不存在，结束
        	"npc/88/8800"->game_over(map,qin?"秦国":"赵国");
}
...
void check_fighter(int i)
{
	...
	level = get("warlevel");
	z = get_id();
	nFighter = "npc/88/8800"->get_2(sprintf("%d.%d.秦国",level,wId));
	nFighter += "npc/88/8800"->get_2(sprintf("%d.%d.赵国",level,wId));
