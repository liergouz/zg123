...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：15级\n    使怪物陷入迷魂状态，甚至被玩家所控。迷魂的成功率会跟随技能等级的提升而提升。";
...
int cast_done( object me )
{
        ...
        if( who->is_badman() )    // 只能控制野外的怪
        {
        	if (who->get_char_type()==FIGHTER_TYPE_2 || who->get("anti_05311"))
                {
                        write_user( me, ECHO "您不能控制%s。", who->get_name() );
                        return 2;    // 执行成功
                }   
        	if (who->get_level()>me->get_level())
                {
                        write_user( me, ECHO "您还不足以控制%s。", who->get_name() );
                        return 2;    // 执行成功
                }                      	
                soldier = me->get("soldier");
                if (objectp(soldier))
                {
                        write_user( me, "您已迷魂了其他怪物。" );
                        return 2;    // 执行成功
                }  
...
//              CHAR_DIE_D->is_enemy_die(me, who, 0);
	        ...
	        if ( who && !who->is_die() )
                ...
                if( rate < 1 )
                {
                        write_user( me, ECHO "您还不足以迷惑%s。", who->get_name() );
                        return 2;    // 执行成功
                }
