...
/*      if( !objectp( item = present("阵旗", me, 1, MAX_CARRY) ) )
        ...
        if( !objectp( item2 = present("黄豆", me, 1, MAX_CARRY*4) ) )
        {
                printf( ECHO "您得有黄豆才能使用“" PF_NAME "”。" );
                return 1;
        }

        if (objectp( me->get("soldier")))
        {
                printf( ECHO "您已召唤了豆兵或者迷魂了怪物。" );
                return 1;
        }
...
int cast_done( object me )
{
        ...
        send_user(me, "%c%c%w%s", 0x5a, 0, 10, "召唤豆兵中……");	                
...
void can_perform( object me )
{
	...
	send_user(me, "%c%w%w%c%c%c%c%c%s", 0x64, THIS_PERFORM, SUB_MP, 0,1, MAGIC_ACT_BEGIN, 0, 9, "黄豆");
}
