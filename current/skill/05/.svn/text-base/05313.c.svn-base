...
int main( object me, object who, int x, int y, string arg )
{
        ...
        if( !objectp( item = present("迷神散", me, 1, MAX_CARRY) ) )
        {
                printf( ECHO "您得有迷神散才能使用“" PF_NAME "”。" );
                return 1;
        }       
        item->add_amount(-1); 
...
void can_perform( object me )
{
	...
	send_user(me, "%c%w%w%c%c%c%c%c%s", 0x64, THIS_PERFORM, SUB_MP, 0,10, MAGIC_ACT_BEGIN, 0, 5, "迷神散");
}
