...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：18级\消耗道具：符纸3张\nn    呼唤火龙烈焰无情地焚烧敌人，提升30点法术攻击力，使敌人陷入12秒的焚烧状态。焚烧状态中，每3秒至少受到30点的伤害。";
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\n消耗道具：符纸3张\n法力消耗：%d点\n冷却时间：%d秒\n    呼唤火龙烈焰无情地焚烧敌人，提升%d点法术攻击力，使敌人陷入12秒的焚烧状态。焚烧状态中，每3秒至少受到%d点的伤害。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    提升%d点法术攻击力，使敌人陷入12秒的焚烧状态。焚烧状态中，每3秒至少受到%d点的伤害。",
	...
	if (who->is_anti_effect()) return 2;
        ...
        if( ret > 0 )
        {
        	...
        	if (get_effect(me, EFFECT_USER_BURN))
        	{
                ...
                set_effect_2(who, EFFECT_USER_BURN, (ADD_INTERVAL) / 3, 3);    // 小心准备 me
...
*/
        ...
        if( random(100) < range_value(rate - who->get_immunity(), 5, 100) )
        {
                ...
                user = get_scene_object_2(who, USER_TYPE);
                set_effect_2(who, EFFECT_USER_BURN, (ADD_INTERVAL) / 2, 2);    // 小心准备 me
                send_user( who, "%c%w%w%c", 0x81, 9026, get_effect_3(who, EFFECT_USER_BURN), EFFECT_BAD );
                send_user( user - ({ who }), "%c%d%w%c%c", 0x83, getoid(who), 9026, 1, EFFECT_BAD );
        }
