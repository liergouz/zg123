...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：25级\n消耗道具：符纸2张\n    可瞬间传送到一个指定ＩＤ的玩家身边。如果该玩家在安全区或者洞穴之中，无法使用该技能传送。";
      ...
      if( !objectp( item = present("符纸", me, 1, MAX_CARRY*4) ) )
        ...
        else
        {
              ...
              case BATTLE_MAP : printf( ECHO "对方在战场，不能使用“" PF_NAME "”。" );
                        ...
                        {
                        	printf( ECHO "对方在副本地图中，不能使用“" PF_NAME "”。" );
                        	return 1;
                        }
                }
...
void can_perform( object me )
{
	...
	send_user(me, "%c%w%w%c%c%c%c%c%s", 0x64, THIS_PERFORM, SUB_MP, 0, 1, MAGIC_ACT_BEGIN, 0, 5, "符纸");
}
