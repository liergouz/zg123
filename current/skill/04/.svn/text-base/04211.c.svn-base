...
#define ADD_INTERVAL    ( 15 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 17 * 5 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：35级\n消耗道具：符纸3张\n    一定几率将一个敌人变成一只小动物，被变化成为小动物的目标不能攻击，且不能使用任何的法术与技能（但可以使用道具）。";
        cur_level = (level - SKILL_LEVEL)/17 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\n消耗道具：符纸3张\n法力消耗：%d点\n冷却时间：%d秒\n    一定几率将一个敌人变成一只小动物，被变化成为小动物的目标不能攻击，且不能使用任何的法术与技能（但可以使用道具）。被变身期间，躲闪值会增加300％，回避率增加50％。\n    变身术对Boss类怪物无效。\n    变身术的成功率会根据敌人的等级作出相应的调整。\n    变身术的效果持续%d秒。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    变身术效果的持续时间延长至%d秒。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_INTERVAL, cur_level*17+SKILL_LEVEL, ADD_INTERVAL+5 );
	return result;
}
