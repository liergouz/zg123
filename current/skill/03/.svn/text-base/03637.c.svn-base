...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：22级\n    追加20点毒性。以自己为中心的7×7格范围释放毒雾，瞬间感染该范围内所有的敌对目标，且被感染的敌人会陷入延缓状态。";
        cur_level = (level - SKILL_LEVEL)/9 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\nn法力消耗：%d点\n冷却时间：%d秒\n    追加%d点毒性。以自己为中心的7×7格范围释放毒雾，瞬间感染该范围内所有的敌对目标，且被感染的敌人会陷入延缓状态。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    追加毒性%d点。", 
...
int main( object me, object who, int x, int y, string arg )
{
	...
	poison = item->get_poison() + ADD_EFFECT;
        me->add_mp(-mp);
        item->add_amount(-1);
        if( objectp( item1 = me->get_equip(WEAPON_TYPE) ) && item1->get_name()=="碧火寒月环" ) 
        	poison += 1000;
