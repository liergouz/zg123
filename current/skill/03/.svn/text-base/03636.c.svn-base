...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：3级\n物品消耗：毒药\n    将毒药涂抹在友方的武器之上，使其每次攻击有40％的几率使敌人中毒。";
...
int main( object me, object who, int x, int y, string arg )
{
	...
	name = item->get_name();
        ...
        if (who!=me)
        {
        	...
        	{
	                printf( ECHO "“" PF_NAME "”只能对同队伍的玩家使用。" );
	                return 1;        		
        	}
        }
...
int cast_done( object me )
{
	...
	if (item)
	{
		...
		if (who!=me)
		{
			write_user( me, ECHO "您将%s涂在%s的武器上。", name, who->get_name() );
			write_user( who, ECHO "%s将%s涂在您的武器上。", me->get_name(), name );
		}
		else
			write_user( me, ECHO "您将%s涂在自己的武器上。", name );
	}
        return 2;    // 执行成功
}
