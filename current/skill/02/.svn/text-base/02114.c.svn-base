...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：34级\n    击落对手武器，使对手在8秒内无法装备使用武器。对怪物使用能减弱其30点物理攻击力。";
        cur_level = (level - SKILL_LEVEL)/16 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\n元气消耗：%d点\n冷却时间：%d秒\n    击落对手武器，使对手在8秒内无法装备使用武器。对怪物使用能减弱其%d点物理攻击力。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    使用冷却时间减少至%d秒。对怪物使用能减弱其%d点物理攻击力。", 
...
void can_perform( object me )
{
	...
	name = sprintf("%s(%d级)", PF_NAME, (me->get_skill(THIS_SKILL)-SKILL_LEVEL)/16+1);
