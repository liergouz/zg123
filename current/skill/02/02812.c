...
#define ADD_HP          ( 15 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 10 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：16级\n    利用旋风攻击远处一个敌人，不断吸取对方15点气血值，持续10秒，法术持续的同时对方移动速度减慢。在组队的时候，可将吸取的气血平均分配给所有21×21范围内的队伍成员。";
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\n法力消耗：%d点\n冷却时间：%d秒\n    利用旋风攻击远处一个敌人，不断吸取对方%d点气血值，持续10秒，法术持续的同时对方移动速度减慢。在组队的时候，可将吸取的气血平均分配给所有21×21范围内的队伍成员。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    持续10秒内不断吸取敌人%d点气血值。", 
...
int main( object me, object who, int x, int y, string arg )
{
	...
	if ( who->get("anti_slow") ) return 2;
        ...
        me->set_enemy(0);
        send_user(me, "%c%c%w%s", 0x5a, 0, 10, "旋风诀施放中……");
