...
#define SUB_MP          0
#define SUB_YUAN        4
#define ADD_TIME	60 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n技能需求：20级\n    枪棍的架势步法，60秒内提升同队所有成员20％的回避率。";
        cur_level = (level - SKILL_LEVEL)/10 + 1;
        result = sprintf(HIC"%s(%d级)\n" NOR "技能需求：%d级\n元气消耗：%d点\n冷却时间：%d秒\n    枪棍的架势步法，%d秒内提升同队所有成员20％的回避率。\n" HIC"下一级：\n"NOR"技能需求：" HIR "%d级" NOR"\n    效果持续时间延长至%d秒。",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_YUAN, TIME_INTERVAL, ADD_TIME, cur_level*10+SKILL_LEVEL, ADD_TIME+1 );
	return result;
}
