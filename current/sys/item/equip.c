...
mapping mpHole = ([ 1:"欧冶宝石",2:"干将宝石",]);	//装备的孔中镶嵌的宝石
...
string get_chinese_prop_string( string file, string prop, int point )
{
        switch( prop ) 
        {
   case "ap"  : return sprintf( "%s攻击: %+d\n", !file->get( "ap" ) ? HIY : point > file->get( "ap" ) ? HIC : NOR, to_int(point) );
  case "app"  : return sprintf( "%s附加攻击: %+d\n", HIC, to_int(point) );
  case "ap2"  : return sprintf( "%s锻造攻击: %+d\n", HIC, to_int(point) );
  case "double"  : return sprintf( "%s暴击率: %+.2f%%\n", HIC, to_float(point)/100 );
 case "double2"  : return sprintf( "%s镶嵌暴击率: %+.2f%%\n", HIC, to_float(point)/100 );
   case "ap%" : return sprintf( "%s攻击: %+d%%\n", !file->get( "ap%" ) ? HIY : point > file->get( "ap%" ) ? HIC : NOR, to_int(point) );
    case "!%" : return sprintf( "%s命中: %+.2f%%\n", !file->get( "!%" ) ? HIY : point > file->get( "!%" ) ? HIC : NOR, to_float(point)/100 );
   case "cp"  : return sprintf( "%s法术攻击: %+d\n", !file->get( "cp" ) ? HIY : point > file->get( "cp" ) ? HIC : NOR, to_int(point) );
  case "cpp"  : return sprintf( "%s附加法攻: %+d\n", HIC, to_int(point) );
  case "cp2"  : return sprintf( "%s锻造法攻: %+d\n", HIC, to_int(point) );
   case "cp%" : return sprintf( "%s法术攻击: %+d%%\n", !file->get( "cp%" ) ? HIY : point > file->get( "cp%" ) ? HIC : NOR, to_int(point) );
   case "c!%" : return sprintf( "%s法术命中: %+d.%%\n", !file->get( "c!%" ) ? HIY : point > file->get( "c!%" ) ? HIC : NOR, to_int(point) );
   case "t!%" : return sprintf( "%s暗器命中: %+d%%\n", !file->get( "t!%" ) ? HIY : point > file->get( "t!%" ) ? HIC : NOR, to_int(point) );
   case "dp"  : return sprintf( "%s防御: %+d\n", !file->get( "dp" ) ? HIY : point > file->get( "dp" ) ? HIC : NOR, to_int(point) );
  case "dpp"  : return sprintf( "%s附加防御: %+d\n", HIC, to_int(point) );
  case "dp2"  : return sprintf( "%s锻造防御: %+d\n", HIC, to_int(point) );
   case "pp"  : return sprintf( "%s法术防御: %+d\n", !file->get( "pp" ) ? HIY : point > file->get( "pp" ) ? HIC : NOR, to_int(point) );
  case "ppp"  : return sprintf( "%s附加法防: %+d\n", HIC, to_int(point) );
  case "pp2"  : return sprintf( "%s锻造法防: %+d\n", HIC, to_int(point) );
   case "hp"  : return sprintf( "%s气血: %+d\n", !file->get( "hp" ) ? HIY : point > file->get( "hp" ) ? HIC : NOR, to_int(point) );
  case "hpp"  : return sprintf( "%s附加气血: %+d\n", HIC, to_int(point) );
  case "hp2"  : return sprintf( "%s锻造气血: %+d\n", HIC, to_int(point) );
   case "hp%" : return sprintf( "%s气血: %+d%%\n", !file->get( "hp%" ) ? HIY : point > file->get( "hp%" ) ? HIC : NOR, to_int(point) );    // 小心 point 是 %f 的情况
   case "mp"  : return sprintf( "%s法力: %+d\n", !file->get( "mp" ) ? HIY : point > file->get( "mp" ) ? HIC : NOR, to_int(point) );
  case "mpp"  : return sprintf( "%s附加法力: %+d\n", HIC, to_int(point) );
  case "mp2"  : return sprintf( "%s锻造法力: %+d\n", HIC, to_int(point) );
   case "mp%" : return sprintf( "%s法力: %+d%%\n", !file->get( "mp%" ) ? HIY : point > file->get( "mp%" ) ? HIC : NOR, to_int(point) );
   case "hp*" : return sprintf( "%s吸取气血: %+d%%\n", !file->get( "hp*" ) ? HIY : point > file->get( "hp*" ) ? HIC : NOR, to_int(point) );
   case "mp*" : return sprintf( "%s吸取法力: %+d%%\n", !file->get( "mp*" ) ? HIY : point > file->get( "mp*" ) ? HIC : NOR, to_int(point) );
   case "sp"  : return sprintf( "%s躲闪: %+d\n", !file->get( "sp" ) ? HIY : point > file->get( "sp" ) ? HIC : NOR, to_int(point) );
   case "sp2"  : return sprintf( "%s锻造躲闪: %+d\n", HIC, to_int(point) );
    case "?%" : return sprintf( "%s回避率: %+.2f%%\n", !file->get( "?%" ) ? HIY : point > file->get( "?%" ) ? HIC : NOR, to_float(point)/100 );
   case "c?%" : return sprintf( "%s法术闪避: %+.2f%%\n", !file->get( "c?%" ) ? HIY : point > file->get( "c?%" ) ? HIC : NOR, to_float(point)/100 );
   case "-*"  : return sprintf( "%s免伤害: %+d\n", !file->get( "-*" ) ? HIY : point > file->get( "-*" ) ? HIC : NOR, to_int(point) );
   case "-*%" : return sprintf( "%s免伤害: %+d%%\n", !file->get( "-*%" ) ? HIY : point > file->get( "-*%" ) ? HIC : NOR, to_int(point) );
  case "-c*"  : return sprintf( "%s抗法术: %+d\n", !file->get( "-c*" ) ? HIY : point > file->get( "-c*" ) ? HIC : NOR, to_int(point) );
  case "-c*%" : return sprintf( "%s抗法术: %+d%%\n", !file->get( "-c*%" ) ? HIY : point > file->get( "-c*%" ) ? HIC : NOR, to_int(point) );
    case "@"  : return sprintf( HIG "毒性: %+d\n", to_int(point) );
    case "@%" : return sprintf( "%s毒性: %+d%%\n", !file->get( "@%" ) ? HIY : point > file->get( "@%" ) ? HIC : NOR, to_int(point) );
   case "X@%" : return sprintf( "%s毒免疫: %+d%%\n", !file->get( "X@%" ) ? HIY : point > file->get( "X@%" ) ? HIC : NOR, to_int(point) );
   case "-@%" : return sprintf( "%s抗毒性: %+d%%\n", !file->get( "-@%" ) ? HIY : point > file->get( "-@%" ) ? HIC : NOR, to_int(point) );
    case "/%" : return sprintf( "%s异常免疫: %+d%%\n", !file->get( "/%" ) ? HIY : point > file->get( "/%" ) ? HIC : NOR, to_int(point) );
   case "??%" : return sprintf( "%s运气: %+d%%\n", !file->get( "??%" ) ? HIY : point > file->get( "??%" ) ? HIC : NOR, to_int(point) );
    case "#%" : return sprintf( "%s可装备: %+d%%\n", !file->get( "#%" ) ? HIY : point > file->get( "#%" ) ? HIC : NOR, to_int(point) );
      default : return "";
        }
}
...
string get_chinese_prop_string_2( string color, string prop, int point )    // 专用于法宝
{
  ...
  case "app"  : return sprintf( "%s附加攻击: %+d\n", HIC, point );
  case "ap2"  : return sprintf( "%s锻造攻击: %+d\n", HIC, point );
  //case "double"  : return sprintf( "%s暴击率: %+d%%%%\n", HIC, point );
  case "double"  : return sprintf( "%s暴击率: %+.2f%%\n", HIC, to_float(point)/100 );
 case "double2"  : return sprintf( "%s镶嵌暴击率: %+.2f%%\n", HIC, to_float(point)/100 );
   case "ap%" : return sprintf( "%s攻击: %+d%%\n", color, point );
