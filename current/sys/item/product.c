...
int get_use_effect(object me, object obj)
{
	...
	{
		send_user( me, "%c%s", '>', HIY"您没有学习相应的技能。");
		return 0;
	}
	if (me->get_skill(skill)<obj->get_product_level())
	{
		send_user( me, "%c%s", '>', HIY"您的技能等级不够，无法学习。");
		return 0;
	}
/*	
...
int make_stuff(object player, object npc, object obj)
{
	...
	{
		tell_user(player, ECHO "此物品没什么有价值的材料。" );
		return 1;
	}
	level = obj->get_level();
	if (player->get_skill(650)==0)
	{
		tell_user(player, ECHO "您不会石刻的技能。" );
		return 1;
	}
	if (player->get_skill(650)<level)
	{
		tell_user(player, ECHO "您的石刻技能等级不足。" );
		return 1;
	}
	if (player->get_strength()<1)
	{
		tell_user(player, ECHO "您没有体力。" );
		return 1;
	}
	if( sizeof_inventory(player, 1, MAX_CARRY) >= MAX_CARRY-1 )
	{
		tell_user(player, ECHO "分解装备道具栏至少需要有两个空位。" );
		return 1;
	}
	...
	if (item)
	{
		tell_user( player,  ECHO "您获得一个%s。", item->get_name() );
		total = 1;
