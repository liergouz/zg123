...
int main( object me, object obj, int count, string sex, object diamond1, object diamond2, object diamond3 )
{
	...
	{
		printf( ECHO "使用“" + obj->get_name() + "”技能等级不足。" );
		return 1;
	}
	// 检查有无相应技能
	if (count<0) 
	{
		printf( ECHO "使用“" + obj->get_name() + "”需指定制作数量。" );
		return 1;
	}
	if ("/sys/item/product"->check_vita_skill(me, obj->get_skill() * 100 + obj->get_number())==0)
	{
		printf( ECHO "你还没学会“" + obj->get_name() + "”。" );
		return 1;
	}
	if (me->get_strength()<obj->get_tili())
	{
		printf( ECHO "使用“" + obj->get_name() + "”体力不足。" );
		return 1;
	}
	...
	for (i=0;i<size;i++)
	{
		...
		{
			printf( ECHO "使用“" + obj->get_name() + "”材料不足。" );
			return 1;
		}
	}
	// 检查工具
	if (obj->get_tool())
	{
		...
		if (!tool)
		{
			printf( ECHO "使用“" + obj->get_name() + "”缺少工具。" );
			return 1;			
		}
		...
		if (file)
		{
			...
			{
				printf( ECHO "使用“" + obj->get_name() + "”的工具不对。" );
				return 1;				
			}
			if (tool_level>tool->get_level())
			{
				printf( ECHO "使用“" + obj->get_name() + "”的工具的等级过低。" );
				return 1;				
			}			
		}
	}
	...
	if (obj->get_master() &&obj->get_master()!="0" )
	{
		...
		if (flag==0)
		{
			printf( ECHO "使用“" + obj->get_name() + "”需要在" + obj->get_master() + "旁边。" );
			return 1;				
		}
	}
	if( sizeof_inventory(me, 1, MAX_CARRY*4) >= allcount )
	{
		printf( ECHO "使用“" + obj->get_name() + "”道具栏至少需要有一个空位。" );
		return 1;
	}	
	final = obj->get_final();
	// 判断如果是普通物品或者生活技能工具，不让使用鲁班宝石
	if (final->get_equip_type()==0 || final->get_equip_type()==HAND_TYPE)
	{
		if (diamond1 && diamond1->is_luban())
		{
			printf( ECHO "使用“" + obj->get_name() + "”不能附加鲁班宝石。" );
			return 1;			
		}
        ...
        send_user(me, "%c%c%w%s", 0x5a, 0, 3, obj->get_name()+"进行中……");	        
        return 1;	
}
...
void into_effect(object me, object obj, string arg)
{
	...
	if (tool)
	{
		...
		{
			printf( ECHO "使用“" + obj->get_name() + "”工具耐久度不足。" );
			obj->effect_break(me, "");
			return;
		}					
	}	
	...
	if (random(100)<rate)
	{
		...
		if (item)
		{
			count = obj->get_count();
			if (count<1) count = 1;		
			printf( ECHO "您使用“" + obj->get_name() + "”炼出" +item->get_name()+ "。" );
	...
	{
		printf( ECHO "使用“" + obj->get_name() + "”失败。" );
