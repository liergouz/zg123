...
int get_use_effect(object me, object obj)
{
	...
	{
		if ( owner )
			send_user( owner, "%c%s", '!', HIY"您的"+me->get_name()+"等级不够，无法服用。");
		else		
			send_user( me, "%c%s", '!', HIY"您的等级不够，无法服用。");
		return 0;
	}
...
{
	string result = "";
	if (me->get("temp_add_hp"))  result +="每两秒恢复气血。\n";
        return result;
}
...
{
	string result = "";
        if (me->get("temp_add_mp"))  result +="每两秒恢复法力。\n";
        return result;
}
