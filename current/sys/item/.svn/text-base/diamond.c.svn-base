        ...
        if( random(100) < rate )    // 合成成功
        {
                ...
                if( level > 7 )    // 完美宝石
                {
                        if( load_object( file = item->get_diamond_file() ) )
                        {
                                ...
                                result += sprintf( "合成一块%s。", item->get_name() );
                ...
                else    result += sprintf( "合成一块%s。", item->get_name() );
...
void make_equip( object me, object item, object item2, object item3, object item4 )
{
        ...
        {
                send_user( me, "%c%s", '!', "只有门派装备才能进行合成。" );
                return;
        }

        if( (level + 1) * 10000 > me->get_cash() )
        {
                send_user( me, "%c%s", '!', "您手上的现金不够。" );
                return;
        }
        if( !( size = sizeof(diamond) ) )
        {
                if( level < 3 )
                        send_user( me, "%c%s", '!', "得有宝石才能进行合成。" );
                else    send_user( me, "%c%s", '!', "得有完美宝石才能进行合成。" );
   ...
   case "-1-" : // 降低装备条件 50%
                ...
                {
                        send_user( me, "%c%s", '!', "得有完美宝石才能进行合成。" );
                        return;
                }

                result = sprintf( "您将 %s同%s", diamond[0]->get_name(), equip->get_name() );
                ...
                if( random(100) < rate )    // 合成成功
                {
                        ...
                        result += sprintf( "合成%s。", equip->get_name() );
   ...
   case "-2-" : // 恢复耐久度 100%
    ...
    case 0..4 : rate = 100;  break;
       ...
       case 9 : rate = 10 + me->get_lucky() / 2 + size * 2 - me->get_pk() * 15;
                ...
                break;
      default : send_user( me, "%c%s", '!', equip->get_name() + "的等级太高了。" );
                return;
                break;
        }
   ...
   case "-2-" : if( !diamond[0]->is_diamond() )    // 必须是完美宝石
    ...
    case 0..4 : rate = 100;  break;
       ...
       case 9 : rate = 10 + me->get_lucky() / 2 + size * 2 - me->get_pk() * 15;
                rate = range_value(rate, 0, 50);
                break;
      default : send_user( me, "%c%c%c%d", 0x25, 0, 0, 0 );    // 合成到了尽头
                return;
                break;
        }
