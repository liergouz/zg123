...
void create()
{
	...
	{
		iStarttw1 = mktime(2007,3,23,0,0,0);	
		iFinishtw1 = mktime(2007,3,28,0,0,0);	
	}
}
// 函数：修正奖励队伍
...
int correct_drop_rate( int level )    // level : 怪 - 玩家等级
{
        ...
        {
        	return 100;
//                if( level <= 5 ) return 100 *10;
//                return range_value( 100 - (level - 5) * 10, 10, 100 );
        }
        else    // 怪 < 玩家
        {
                level = abs(level);
                if( level <= 20 ) return 100 *12;
                return range_value( 100 - (level - 20) * 10, 10, 100 );
        }
}
...
//---------------------------------------------------------------------------------
	...
	if ( iTime > iStarttw1 && iTime < iFinishtw1 )
	{
        ...
        if( size > 1 )    // 多人
        {
			...
			{
				send_user(team[i],"%c%s",';',"由于你处在团队之中，因而无法获得怪物经验和潜能！");	
				continue;
			}
                        ...
                        if (team[i]->get_save_2("refine.status")==1 && random100()<team[i]->get_online_rate())
                        {
                        	...
                        	if (level3>=team[i]->get_save_2("refine.level1") && level3<=team[i]->get_save_2("refine.level2"))
                        	{
                        		if (team[i]->get_save_2("refine.count")<450)
                        		{
	                        		...
	                        		if (team[i]->get_save_2("refine.count")>=450) send_user( team[i], "%c%c%c%w%w%s", 0x51, 1, 2, TID_REFINE, 1, "收妖壶任务(完成)" ); 
	                        		...
	                        		if (team[i]->get_save_2("refine.count")<450) send_user(team[i], "%c%s", '!', sprintf("收妖任务：%d/450", team[i]->get_save_2("refine.count")));
	                        		else send_user(team[i], "%c%s", '!', "收妖任务完成");
	                        	}
                        	}
                        }
                        //大胖任务
...
//                        }
        ...
        else    // 单人
        {
        	if ( who->get_group_id() )
		{
			send_user(who,"%c%s",';',"由于你处在团队之中，因而无法获得怪物经验和潜能！");	
			return ;
		}
                ...
                if (who->get_save_2("refine.status")==1 && random100()<who->get_online_rate())
                {
                	...
                	if (level3>=who->get_save_2("refine.level1")&& level3<=who->get_save_2("refine.level2"))
                	{
                		if (who->get_save_2("refine.count")<450)
                		{
	                		...
	                		if (who->get_save_2("refine.count")>=450) send_user( who, "%c%c%c%w%w%s", 0x51, 1, 2, TID_REFINE, 1, "收妖壶任务(完成)" ); 
	                		send_user( who, "%c%c%c%w%w%s", 0x51, 1, 3, TID_REFINE,1, "" );
	                        	...
	                        	if (who->get_save_2("refine.count")<450) send_user(who, "%c%s", '!', sprintf("收妖任务：%d/450", who->get_save_2("refine.count")));
	                        	else send_user(who, "%c%s", '!', "收妖任务完成");
	                        }
                	}
                }  
                //大胖任务
