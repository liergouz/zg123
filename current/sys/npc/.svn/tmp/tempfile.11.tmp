...
void fight_bonus(object owner, object npc, int team_size, int doubletime,int extra)
{
	...
	{
		send_user(owner,"%c%s",'>',"您的宠物"+pet->get_name()+"目前等级到达上限，参战将不再获取经验！");
		return ;
	}
...
void add_giftpoint(object pet, string arg)
{
	...
	{
		send_user(pet->get_owner(),"%c%s",'!',"点数不够分配！");
		return;
	}
...
int upgrade(object pet)
{
	...
	if( level < pet->get_max_pet_level() )
	{
		...
		pet->add_dex(1);
		set_pet_info(pet);
		send_user( owner, "%c%s", '!', sprintf("你的宠物%s升到%d级！",pet->get_name(),level) );
...
void count_max_hp( object me )
{
	...
	hp = me->get_con()*(me->get_level()*4+nProp[0]*10)/100*me->get_con_grow()/1000;
        ...
        if ( me->get("sk74213") )	//高级御命术
        	...
        	hp += me->get("sk74122");
        if ( get_effect(me, EFFECT_MAGIC_CARD) &&  me->get_save("change_shape") == 501 )	//变身虎王
        	hp += hp;     
        me->set_max_hp(hp);
        if ( me->get_hp() > hp )
        	me->set_hp(hp); 
...
void fuse_item(object pet, object obj,int iFlag)
{
	...
	{
		send_user(owner,"%c%s",'!',"已经摆摊上架的物品不能合成！");
		return ;	
	}
	owner = pet->get_owner();
	cName = pet->get_name();
	if ( obj->get_name() == "玉灵液" || obj->get_name() == "玉灵露"  )	//合成玉灵液
	{
		if ( pet->get_save("no_fuse") )
		{
			send_user(owner,"%c%s",'!',obj->get_name()+"不可使用在特殊的宠物身上。");
			return ;	
		}
		if ( iFlag == 0 )
		{
			send_user(owner,"%c%c%w%s",':',3,0,
				sprintf("即将对%d级的宠物%s使用玉灵液，%s将获得轮回重生，确认是否继续？\n"ESC"确定\npet %x# fuse_y %x#\n"ESC"离开",pet->get_level(),pet->get_name(),pet->get_name(),getoid(pet),getoid(obj)));
			return ;
		}
		//清除所有的技能
		...
		pet->save();
		obj->add_amount(-1);
		log_file("pet.txt",sprintf("%s %s(%d) 洗宝宝 %s(%s) %s\n",short_time(),owner->get_name(),owner->get_number(),pet->get_name(),pet->get_firstname(),pet->get_pet_id()));
		send_user(owner,"%c%s",'!',sprintf("您的宠物%s已经接受了轮回，现已获得了重生～！",cName));
		"/sys/sys/count"->add_pet("洗宝宝", 1);	
		return ;
	}
	else if ( obj->get_name() == "精元丹" )	//合成精元丹
	{
/*		if ( pet->get_save("no_fuse") )
		{
		...
		{
			send_user(owner,"%c%c%w%s",':',3,0,
				sprintf("即将对宠物%s使用精元丹，确认是否继续？\n"ESC"确定\npet %x# fuse_yy %x#\n"ESC"离开",pet->get_name(),getoid(pet),getoid(obj)));
			return ;
		}
		skill = to_int(obj->get("special"));
		if ( pet->get_skill(skill) )
		{
			send_user(owner,"%c%s",'!',"你的宠物已经具有这样的技能了！");
			return ;
		}
		...
		if ( size >= 3 && oSki->get_type() == 1  )	//主动技能是否达到3个？
		{
			...
			if ( (count=sizeof(nSki1)) >= 3 )
			{
				...
				{
					send_user(owner,"%c%c%w%s",':',3,0,
						sprintf("你的宠物 "HIR+pet->get_name() +NOR" 已经拥有三个主动攻击能力了，强行加入新能力只会取代旧能力，确认是否强行合成？\n"ESC"确定\npet %x# fuse_y %x#\n"ESC"离开",getoid(pet),getoid(obj)));
					return ;
				}
				else	//取代已有的之一主动技能
				{
					...
					tell_user(owner,"你的宠物 "+cName+" 领悟了新技能："+name1+"，但是失去了技能："+name2);
					obj->remove_from_user();
					destruct(obj);						
					log_file("pet.txt",sprintf("%s %s(%d) 精元丹 %s(%s,%s) 领悟 %s 失去 %s\n",short_time(),owner->get_name(),owner->get_number(),pet->get_name(),pet->get_firstname(),pet->get_pet_id(),name1,name2));
					pet->save();
					"/sys/sys/count"->add_pet("合成技能"+name1, 1);	
					return ;	
				}
			}
		}
	...
	else if ( obj->get_name() == "吸魂珠" )	//合成吸魂珠
	{
		if ( pet->get_save("no_fuse") )
		{
			send_user(owner,"%c%s",'!',"特殊的宠物无法提取技能。");
			return ;	
		}
		if ( !iFlag )
		{
			send_user(owner,"%c%c%w%s",':',3,0,
				sprintf("你是否要提取宠物 "HIR+pet->get_name() +NOR" 的技能，当技能被提取后，你的宠物会因此而消失～！\n"ESC"确定\npet %x# fuse_y %x#\n"ESC"离开",getoid(pet),getoid(obj)));
			return ;
		}
		if ( pet->get_blood() != 1 )
		{
			send_user(owner,"%c%s",'!',"只有宠物宝宝才能进行技能提炼！");
			return ;	
		}
		if ( pet->get_level() < 50 )
		{
			send_user(owner,"%c%s",'!',"只有达到50级的宠物宝宝才能进行技能提炼！");
			return ;	
		}
		mpSki = pet->get_skill_dbase();
		size = sizeof(mpSki);
		if ( size < 2 )
		{
			send_user(owner,"%c%s",'!',"宠物的技能不能少于2个！");
			return ;	
		}
		nSki = keys(mpSki);
		obj->remove_from_user();
		destruct(obj);						
		log_file("pet.txt",sprintf("%s %s(%d) 吸魂珠 %s(%s) %s\n",short_time(),owner->get_name(),owner->get_number(),pet->get_name(),pet->get_firstname(),pet->get_pet_id()));
		...
		if ( name1 )
		{
			tell_user(owner,"恭喜你得到 "HIY+name1+ NOR"!");
			send_user(owner,"%c%s",'!',"恭喜你得到 "HIY+name1+ NOR"!");
			log_file("pet.txt",sprintf("%s %s(%d) 吸魂珠 得到 \n",short_time(),owner->get_name(),owner->get_number(),name1));
			"/sys/sys/count"->add_pet("提取技能"+name1, 1);	
		}
		else
			tell_user(owner,"你什么也没得到，白忙活了一场！ ");
		return ;
	}
}
//合成
