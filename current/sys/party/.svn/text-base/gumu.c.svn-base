...
void do_look(object who,object me)
{
	...
	{
		result=sprintf(me->get_name()+":\n相传秦国一位大将死后，秦王赏赐奇世珍藏作为其陪葬之品，也就是这些奇世珍藏吸引了无数勇者前去探险，结果都因破不了墓中石俑阵，无功而返！你也想去试一下吗？(请在每天20：30——22：00来我这里申请进入)\n");
		result+=sprintf(ESC "离开");
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),result);
		return;
	}
	result=sprintf(me->get_name()+":\n相传秦国一位大将死后，秦王赏赐奇世珍藏作为其陪葬之品，也就是这些奇世珍藏吸引了无数勇者前去探险，结果都因破不了墓中石俑阵，无功而返！你也想去试一下吗？\n");
	result+=sprintf(ESC "前去古墓探险\ntalk %x# welcome.1\n",getoid(me));
	result+=sprintf(ESC "离开");
	send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),result);
}
...
void do_welcome(object me, object who,string arg)
{
	...
	switch(flag)
	{
		case 1:
		     ...
		     if(size<3)
		     {
		     	      result=sprintf(me->get_name()+":\n    探险可不是儿戏，我还是劝你多寻几个人一起前往，至少相互有个照应！\n");
		     	      result+=sprintf(ESC"离开");
		     	      send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),result);
		     	      return;		     	          	      
		     } 
		     ...
		     map = TEAM_D->get_copy_scene(tid,COPY_GUMU);
				...
				{
					team[i]->set("me_die",1);
					send_user(team[i], "%c%c%w%s", 0x5a, 0, 3, "传送中……");
...
int new_copy_scene(object who,object me)
{
	...
	team=who->get_team();
	size=sizeof(team);
	z = COPY_CTROL->new_scene(d,FILE_MAP,"古墓");
	if(!z) return 0;
...
//	npc->set_name(sprintf("守墓石俑%d",key));
...
//	map=TEAM_D->get_copy_scene(tid,COPY_GUMU);
	...
	for(i=0;i<size;i++)
	{
		send_user(team[i],"%c%s",'!',sprintf("当前击杀数量："HIY" %d/200",key));
	...
	if(count==200)
	{
		...
		for(i=0;i<size;i++)
		{
			send_user(team[i],"%c%s",'!',HIY"请注意：守墓石将，已经苏醒！！！");
		}
		xy=zoubiao[key];
...
int add_npc(object who,int z,int x,int y,int level) //加入小怪
{
	...
	{
		send_user(who,"%c%s",'!',HIY"请注意：守墓石将，已经苏醒！！！");
		return;
	}
	npc=new(FILE_GUAI);
	if(!npc) return 0; 
	...
	NPC_ENERGY_D->init_level(npc,level);
        npc->set_fight_status(level);
        map->add("copy_npc_count",1);
        //npc->set_name(sprintf("守墓石俑%d",map->get("copy_npc_count")));	//用于测试
...
void player_exit()
{
	...
	for(i=0;i<size1;i++)
	{
		...
		for(j=0;j<size2;j++)
		{
			if(!p=get_valid_xy( z, x, y, IS_CHAR_BLOCK)) return;
			x = ( p % 1000000 ) / 1000;  y = p % 1000;
			send_user(member[j],"%c%s",'!',HIY"你已超过探险时间，因此被一阵大风刮出古墓！");
