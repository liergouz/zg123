...
int do_join( object who, object me, string arg )
{
        ...
        {
                send_user( who, "%c%s", '!', "您已经是“" + name1 + "”帮众了。" );
                return 0;
        }        
        ...
        {
                send_user( who, "%c%s", '!', "加入“" + name2 + "”得有" + name4 + "户籍或者无国籍。" );
                return 0;
        }
        send_user( who, "%c%s", '!', "您申请加入“" + name2 + "”。" );
...
void do_accept( object who, object me, string arg )
{
        ...
        if( stringp(name1) && stringp(name2) && name1 == name2 && who->get_org_position() >= ORG_XIANGZHU )    // 本帮官员
        {
                ...
                {
                        send_user( who, "%c%s", '!', "“" + name2 + "”的人数太多了。" );
                        return;
                }
                ...
                if( stringp(me->get_join_id(number)) && sizeof(me->get_join_id(number)) >= 3 )
                {
                        ...
                        {
                                send_user( who, "%c%s", '!', me->get_join_name(number) + "已经是“" + name3 + "”帮众了。" );
                        ...
                        char->save();
                        char->log_legend( sprintf( "您加入“%s”！", name2 ) );			
                        ...
                        FAMILY_D->org_channel( name2, 0, sprintf( HIR "%s接收%s(%d)加入本帮！",
        	...
        	send_user( who, "%c%s", '!', "你没有接受别人申请加入帮派的权限。" );
}
...
void do_refuse( object who, object me, string arg )
{
        ...
        if( stringp(name1) && stringp(name2) && name1 == name2 && who->get_org_position() > ORG_TANGZHU )    // 本帮官员
        {
        	...
        	send_user( who, "%c%s", '!', "你没有拒绝别人申请加入帮派的权限。" );
}
