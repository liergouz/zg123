 ...
 */
        ...
        &&      sizeof(user) )    // 用户个数大于０
        {
                send_user( me, "%c%s", '!', "您已经在游戏中了！" );    // 同时新登录的情况
                db_user_clear( me, me->get_id() );
                destruct(me);
                return;
        }
...
/*	item = new ("/item/01/0001");
	...
	send_user( me, "%c%c%c%w%s", 0x51, 2, 1,TID_NEWBIE_00,"指引任务" );
	send_user( me, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_00,1,"送信使命" );
...
void add_pet(object me)
{
	...
	object pet;	
	string cName,*nKey;
	mapping mpPet1 = (["小山鸡":10,"小野兔":11,"小蜜蜂":150,"小蚂蚁":151]);
	mapping mpPet2 = ([ "鬼火":901,"幽灵":900,"飞贼":202,"恶老头":250,"恶屠":200,"狂徒":201 ]);
