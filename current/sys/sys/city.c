...
object get_city_object( object who )
{
        ...
        {
  case "齐国" : return load_object( CITY_QI );
  case "韩国" : return load_object( CITY_HAN );
  case "赵国" : return load_object( CITY_ZHAO );
  case "魏国" : return load_object( CITY_WEI );
  case "秦国" : return load_object( CITY_QIN );
  case "楚国" : return load_object( CITY_CHU );
  case "燕国" : return load_object( CITY_YAN );
...
void city_vote( object city )
{
        ...
        if( day > 0 && city->get_save_2("elect.day") != day )
        {
                ...
                &&    ( id = city->get_save_2("elect.first") ) )
                {
                        ...
                        result = sprintf( HIY "%s以 %d 票领先赢得大选，成为新一届%s城主！", 
                ...
                result = sprintf( HIY "%s开始城主选举啦，请大家踊跃参选、投票！", city->get_name() );
                FAMILY_D->city_channel( city->get_name(), 0, result );
        }
}
...
void city_policy( object city )
{
        ...
        &&      gone_time( city->get_save_2("policy.time") ) > 86400 )
        {
                switch( flag )
                {
               case 1 : result = sprintf( HIY "２４小时期限已到，%s招贤令自动取消！", city->get_name() );
                        FAMILY_D->city_channel( city->get_name(), 0, result );
                        break;
              case -1 : result = sprintf( HIY "２４小时期限已到，%s逐客令自动取消！", city->get_name() );
                        FAMILY_D->city_channel( city->get_name(), 0, result );
                        break;
                }
                ...
                &&    ( city2 = get_city_object_2(i) ) )
        {
                if( city->get_relation(i) == 2 )    // 同盟关系
                {
                        ...
                        result = sprintf( HIY "由于友好度过低，%s与%s的同盟被自动解除。", city->get_name(), city2->get_name() );
                        FAMILY_D->city_channel( city->get_name(), 0, result );
                        result = sprintf( HIY "由于友好度过低，%s与%s的同盟被自动解除。", city2->get_name(), city->get_name() );
