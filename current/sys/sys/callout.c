...
//        "/sys/item/stuff3"->reset_stuff();
	...
	"sys/party/distribute"->start_it();
	"sys/party/football"->check_time();
//	"sys/party/duanwujie"->check_time();
						...
						// 帮派解散，处理帮众
			                        ...
			                        for( k = 0; k < size2; k ++ )
			                        {
			                        	...
			                        	if( objectp( user = find_player(name2[k]) ) )
			                                ...
			                                "/inh/user/mail"->sys_mail(id, number, sprintf("由于%s帮派经营不善，难以为继，特此解散。", name[i]));
			                        }
			                        ...
			                        destruct(orgnpc);
			                        CHAT_D->rumor_channel(0,HIR+sprintf("据说由于%s帮派经营不善，难以为继，被迫解散了。", name[i]));
					}
					else
					{
...
*/						
						...
						if (time>0 && time()>time)
						{
							...
							FAMILY_D->org_channel( name[i], 0, sprintf( HIR "经过天机院的不懈努力，帮派研究力顺利提升了1点，现帮派的研究力为%d。",
                                         			orgnpc->get_science() ) );
						}
						...
						if (time>0 && time()>time)
						{
							...
							FAMILY_D->org_channel( name[i], 0, sprintf( HIR "帮派规模上升1，现帮派的规模为%d。",
								orgnpc->get_grade() ) );
						}						
							...
							else
							{
								...
								if (time % 2 ==0)
								{
									number = orgnpc->get_demissnumber();
						                        FAMILY_D->org_channel( name[i], 0, sprintf( HIR "帮主%s对帮中的%s(%d)青睐有加，计划让位与其，在%d小时之后，禅让将完成。",
						...
						if (counter % 2==0)
						{
							...
							if (orgnpc->get_gold()<gold||orgnpc->get_store()<store)
							{
								if (orgnpc->get_gold()<gold)
									result = "帮派总管：本帮由于资金不足，导致帮派安定度下降，已经达到难以为继的地步，望帮中成员多多努力。\n";
								else
									result = "帮派总管：本帮由于储备不足，导致帮派安定度下降，已经达到难以为继的地步，望帮中成员多多努力。\n";
							...
							else
							{
								...
								result = sprintf("帮派总管：这次帮派日常维护，一共消耗了%d帮派资金和%d点帮派储备。帮派资金仍剩余%d，帮派储备仍剩余%d点。", gold, store, orgnpc->get_gold(), orgnpc->get_store() );
								FAMILY_D->org_channel( name[i], 0, HIR +result );
							}
						}							
						orgnpc->save();
