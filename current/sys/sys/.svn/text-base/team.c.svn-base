...
mapping mpGainMode = ([0:"自由拣取",1:"队长分配",2:"队伍分配",3:"轮流拣取",4:"需求分配",]);
...
int change_leader( object leader, object who )
{
	...
	set_save_2(sprintf("%d.member",tId),nObj);
		...
		if ("/sys/sys/count"->get_new_person())
                ...
                send_user( nObj, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "现在队伍的新队长是%s(%d)。", who->get_name(), who->get_number() ) );
...
void change_gain_mode(object who,string id,string arg)
{
	...
	if ( com == "mode" )
	{
		...
		send_user( nObj, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "现在队伍的分配方式是%s。", mpGainMode[mode] ) );
	}
	else if ( com == "type" )
	{
		if ( mode < 1 || mode > 2 )
		...
		send_user( nObj, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "现在队伍分配的是%s装备。", mode==1?"蓝色以上":"金色" ) );
	}
}
...
int give_item(object me,object who,object item)
{
	...
	if ( (owner=environment(item)) && owner->is_user() )	//被拾取
		return 0;
        if( ( total = USER_INVENTORY_D->can_carry_3(who, item) ) < 1 )
        {
        	if ( me == who )
                	...
                	send_user(me, "%c%s",'!',who->get_name()+"携带的东西太多了。" );
                return 0;
        }
	item->delete("team_id");
                ...
                for(i=0,size=sizeof(member);i<size;i++)
               	{
               		...
               		if ( !me )
               			continue;
	                tell_user(me, "您捡起 %d 金。", total/size );
	                me->add_cash2(total/size);  me->add_gold_log("get", total/size);
	                me->log_money("捡", total/size);
	                if (from)
	                {
				result = sprintf("拾取 %s %s,%d,%s,%s 金 0 %d 0 %d_%d_%d", from, me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), total/size, get_z(me), get_x(me), get_y(me) );
				"/sys/user/cmd"->log_item(result);
			}	                
	        }
...
//		);
	...
	else if ( mode == 3 ) 	//轮流拣取
	{
		...
		if ( i <= size-1)
		{
			...
			{
				send_user( member, "%c%c%d%s", 0x43, 7, 0,WHT+me->get_name()+"得到"+name);
...
int leader_give_item(object me, string arg)
{
	...
	{      
        	tell_user(who, "您捡起 %s。", name );
		send_user( nObj, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "%s将"HIR"%s"WHT"分配给了%s", me->get_name(),name,who->get_name() ) );
        }	
	return 1;	
}
...
void distribute_item_call()
{
	...
	for(i=0;i<size;i++)
	{
		...
		{
			nObj = who->get_team();
	        	tell_user(who, "您获得 %s。", name );
			send_user( nObj, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "%s 得到了 "HIR"%s", who->get_name(),name ) );
		}
	}
	nId -= nId1;
...
void add_bet(object who , string arg)
{
	...
	send_user( team, "%c%c%d%s", 0x43, 7, 0, sprintf( WHT "%s 参与竞投%s，获得点数为 %d",who->get_name(),item->get_name(),rate));
...
int check_equip(object me,object equip)
{
	...
	if(  index == FAMILY_TYPE ) return 0;	
		...
		default : switch( me->get_fam_name() )
			...
			case "桃花源" : if( flag != 1 ) flag = -1;  break;                  
			case "蜀山" : if( flag != 2 && flag!=9 ) flag = -1;  break;
			case "禁卫军" : if( flag != 3 ) flag = -1;  break;
			case "唐门" : if( flag != 4 ) flag = -1;  break;
			case "茅山" : if( flag != 5 && flag!=9 ) flag = -1;  break;
			case "昆仑山" : if( flag != 6 && flag!=9 ) flag = -1;  break;                
			case "云梦泽" : if( flag != 7 && flag!=9 ) flag = -1;  break;
                        }
                        break;
		...
		case HAND_TYPE : switch( me->get_fam_name() )
			...
			case "云梦泽" : if( flag != 7 ) flag = -1;  break;
			case "唐门" : if( flag != 4 ) flag = -1;  break;
                        }
                        break;
                }
        ...
        else
        {
			...
			switch( me->get_fam_name() )
                        {
			case "桃花源" : if( type != UNARMED ) flag = -1;  break;                  
			case "蜀山" : if( type != SWORD && type!=BLADE  ) flag = -1;  break;
			case "禁卫军" : if( type != STICK && type!=SPEAR ) flag = -1;  break;
			case "唐门" : if( type != THROWING ) flag = -1;  break;
			case "茅山" : if( type != SWORD ) flag = -1;  break;
			case "昆仑山" : if( type != SWORD ) flag = -1;  break;                
			case "云梦泽" : if( type != SWORD ) flag = -1;  break;
                        }        		
        	}
                if( flag < 0 )
		        return 0;
