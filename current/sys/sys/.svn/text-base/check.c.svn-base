...
int is_legal_id( object me, string id )
{
        ...
        if( !stringp(id) || id == "" )
        {
                send_user( me, "%c%s", '!', "您的帐号必须是由３～２０个字符组成。" );
                return 0;
        }
...
int is_legal_passwd( object me, string passwd )
{
        ...
        {
                send_user( me, "%c%s", '!', "您的密码必须是由５～１４个字符组成。" );
                send_user(me, "%c%c", 0xff, 0x11); 
                return 0;
        }
...
int is_legal_name( object me, string name )
{
        ...
        {
                send_user( me, "%c%s", '!', "您的名字必须是由１～６个中文组成。" );
                send_user(me, "%c%c", 0xff, 0x11); 
                return 0;
        }
        if( !is_chinese(name) )
        {
                send_user( me, "%c%s", '!', "您的名字不合规范。" );
                send_user(me, "%c%c", 0xff, 0x11); 
                return 0;
        }
        if( len < 2 || len > 12 )
...
int is_legal_org( object me, string name )
{
        ...
        if( !stringp(name) || name == "" )
        {
                send_user( me, "%c%s", '!', "帮派名称必须是由１～５个中文组成。" );
                return 0;
        }
        if( !is_chinese(name) )
        {
                send_user( me, "%c%s", '!', "帮派名称不合规范。" );
                return 0;
        }
        if( len < 2 || len > 10 )
        {
