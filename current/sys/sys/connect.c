...
//      me->add_pk(0);    // 显示ＰＫ状态    // 改到 cmd.c 处理
     ...
     case JOB_SCHOLAR : if( file = load_object( SKILL->get_skill_file(SKILL_SCHOLAR) ) ) file->can_perform(me);  break;
      case JOB_TRADER : if( file = load_object( SKILL->get_skill_file(SKILL_TRADER) ) ) file->can_perform(me);  break;
        ...
        // 加上防沉迷的提示
        if (me->get_online_time()>5*3600) send_user( me, "%c%c%d%s", 0x43, 10, 0, HIY "您已进入不健康游戏时间，为了您的健康，请您立即下线休息。如不下线，您的身体将受到损害，您的收益已降为零，直到您的累计下线时间满5小时后，才能恢复正常。" );
        else
        if (me->get_online_time()>3*3600) send_user( me, "%c%c%d%s", 0x43, 10, 0, HIY "您累计在线时间已满3小时，请您下线休息，做适当身体活动。" );
}
