...
int check_show_beast(object me, object pet)
{
	...
	{
		send_user(me,"%c%s",'!',"以你现在的能力还不足以驾驭该宠物。");
		return 0;
	}
	if ( pet->get_life() < 100 )
	{
		send_user(me,"%c%s",'!',sprintf("您的宠物%s自知天命，所剩时日不多，拒绝参加战斗～！",pet->get_name()));
		return 0;
	}
	if ( pet->get_faith() < 10 )
	{
		send_user(me,"%c%s",'!',sprintf("您的宠物%s忠诚度实在太低，还是提升了忠诚度再召唤吧！",pet->get_name()));
		return 0;
	}

	if ( pet->is_die() )
	{
		send_user(me,"%c%s",'!',sprintf("您的宠物%s还没复活，不能召唤！",pet->get_name()));
		return 0;
	}
	if ( (time=pet->get_dead_time()) && (time=gone_time(time)) < 600 )
	{
		send_user(me,"%c%s",'!',sprintf("您的宠物%s因为上次战斗元气大伤，还需%d分钟才能恢复。",pet->get_name(),10-time/60));
		return 0;
	}
	return 1;	
}
...
void show_beast_perform(object me,object pet)
{
	...
	if ( check_show_beast(me,pet) != 1 )
		return ;		
        send_user(me, "%c%c%w%s", 0x5a, 0, 10, "召唤宠物中……");	                
...
void show_beast(object me,object pet)
{
	...
	{
		send_user(me,"%c%s",'!',sprintf("您的宠物%s忠诚度过低，不愿意参加战斗！",pet->get_name()));
		return ;
	}
	if ( pet->get("show") )
		return ;
...
//	pet->set_head_color(npc->get_head_color());
	...
	pet->send_info();
	log_file("pet.txt",sprintf("%s %s(%d) 捕抓 %s %s 宝宝:%d\n",short_time(),me->get_name(),me->get_number(), pet->get_name(),pet->get_pet_id(),baobao));
	if (baobao)
		"/sys/sys/count"->add_pet("捕抓"+pet->get_name()+"宝宝", 1);	
	else
		"/sys/sys/count"->add_pet("捕抓"+pet->get_name(), 1);	
	return 1;	
}
//删除宠物
...
string change_name(object owner,object pet,string cName) 
{
	...
	if ( sizeof(cName) > 12 )
	{
		send_user(owner,"%c%s",'!',"不能超过六个汉字的长度！");
		return;	
	}
	if ( strstr(cName,"宝宝") != -1 || strstr(cName,"|") != -1 
		|| strstr(cName,"%") != -1 || strstr(cName,".") != -1 )
	{
		send_user(owner,"%c%s",'!',"非法字符！");
...
int use_shenshou_item(object who, object item)
{
	...
	{
		send_user(who, "%c%s", '!', "你的宠物空间已满，无法增加新的宠物");
		return 0;
	}		
	name = item->get_name();
	...
	pet->send_info();	
	send_user(who, "%c%s", '!', "你得到神兽"HIR+pet->get_name());
	return 1;	
}