...
void send_max_carry_flag( object me )
{
        ...
        d = bag / 10000;
/*
        if( bag & BAG_TYPE_1 ) b = MAX_CARRY_2;    // 包裹
        if( bag & BAG_TYPE_2 ) c = MAX_CARRY;    // 百宝囊
        if( bag & BAG_TYPE_3 ) d = MAX_CARRY;    // 乾坤袋
*/
	if (b>0)
	{
...
int send_carry_desc(object me, int type)
{
	...
	bag = me->have_bag();
        ...
        if (limit==0) return 1;
        if (limit==-1) result = "永不过期";
        else
        if (time()>limit) result = sprintf(HIR"%s已超过使用时限。\n", result);
        else result = sprintf("%s还有 %d 天的使用期限。\n", result, (limit-time())/(24*3600));
