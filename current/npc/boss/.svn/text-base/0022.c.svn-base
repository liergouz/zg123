...
//        set("no_sk04237",1);	//玄龙决对其没有效果
	...
	log_file("boss.txt",short_time()+" 飞贼王出现\n");
}
...
//	CHAT_D->rumor_channel( 0, CHAT+"听说凶猛的虎王已经被勇敢的侠士所消灭" ); 
	log_file("boss.txt",short_time()+" 飞贼王被消灭\n");
...
//	if ( rate < rate1 )
	{
		...
		if ( random(100) < 50)
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//战国令
		{
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//战国号角
		{
	...
	else if ( rate < 9500  * rate1 / 100)	//战国号角
	{
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	
		{
			item = new( "/item/08/0002" );
	...
	else if ( rate < 10000  * rate1 / 100)	//战国令
	{
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	
		{
			item = new( "/item/08/0001" );
...
{
	send_user( who, "%c%c%w%s", ':',3,0202,"飞贼王:\n    愚蠢的人类！竟敢踏入本王领土！！看来你活得不耐烦了！！\n"+
		sprintf(ESC"挑战\ntalk %x# fight\n",getoid(this_object())) +
		ESC"离开");
}
