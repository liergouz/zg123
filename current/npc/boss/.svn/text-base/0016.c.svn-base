...
void create()
{
	...
	set("anti_05311",1);    //不会迷魂
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"据说阴间叛徒又出现在了万人坑的最深层。" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
        set("guard",({}) );
	log_file("boss.txt",short_time()+"阴间叛徒出现\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(10)>1 )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "我好恨啊…………我好恨啊…………"); 
						break;
					case 1:
						SAY_CMD->say(me, enemy->get_name()+"，你的怨恨我已经全部接收了。"); 
						break;
					default:
						SAY_CMD->say(me, "我已经感受到你们内心的怨恨了。"); 
						break;
				}
			}
		}
...
//		if (number<5) number++;
	...
	{
		silent = time();
		SAY_CMD->say(me, who->get_name()+"，死亡的拥抱不一定是冰冷的。"); 
		return "skill/11/11012"->perform_action_npc(me, who);//冰霜咒	
	}
	if ( gone_time(silent2) >= 30)
	{
		silent2 = time();
		SAY_CMD->say(me, who->get_name()+"，你这可悲的家伙，想死一次吗？"); 
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+sprintf("听说阴间叛徒已经被%s净化了",who->get_name()) ); 
	log_file("boss.txt",short_time()+"阴间叛徒被消灭\n");
...
{
	send_user( who, "%c%c%w%s", ':',3,1255,"阴间叛徒:\n    没人会了解我的怨恨……没人会了解我的怨恨……\n"+
		sprintf(ESC"挑战\ntalk %x# fight\n",getoid(this_object())) +
		ESC"离开");
}
