...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"听说凶猛的虎王再次出现在长白天池！" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" 虎王出现\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "来啊！来啊！！让你们好好见识一下本王的实力！！"); 
						break;
					case 1:
						SAY_CMD->say(me, "弱小的人类！受死吧！"); 
						break;
					case 2:
						SAY_CMD->say(me, enemy->get_name()+"，看本王如何将你剁成肉酱！！"); 
						break;
					default:
						SAY_CMD->say(me, "本王的武功天下第一！无人能伤害得了我！！"); 
						break;
				}
			}
		}
...
void win_bonus_callout( object me, object who )
{
	...
	log_file("boss.txt",short_time()+" 虎王被消灭\n");
...
{
	send_user( who, "%c%c%w%s", ':',3,0501,"虎王:\n    愚蠢的人类！竟敢踏入本王领土！！看来你活得不耐烦了！！\n"+
		sprintf(ESC"挑战\ntalk %x# fight\n",getoid(this_object())) +
		ESC"离开");
}
