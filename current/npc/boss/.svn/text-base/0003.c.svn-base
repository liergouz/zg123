...
string *soldier = ({ "士兵一号","士兵二号","士兵三号","士兵四号","士兵五号","士兵六号","士兵七号","士兵八号", });
mapping mpSoldier = ([]);
// 函数：是否不能进城
...
void create()
{
	...
	init_fight_status();
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"据说威武大将军正带着他的部下在牧场练兵。" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
        set("guard",({}) );
	log_file("boss.txt",short_time()+" 威武大将军出现\n");
}
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(100) < 80 )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, enemy->get_name()+"，就凭你这付病猫相也敢挑战本将军！！"); 
						break;
					case 1:
						SAY_CMD->say(me, "来一个砍一个！来两个杀一双！！"); 
						break;
					case 2:
						SAY_CMD->say(me, enemy->get_name()+"，我要你瞧瞧本将军剑法的厉害！"); 
						break;
					default:
						SAY_CMD->say(me, "士兵们！把腰杆挺直！！随时准备全军突击！！"); 
						break;
				}
			}
		}
...
void summon_guard()
{
	...
	for(i=0,size=sizeof(soldier);i<size;i++)
	{
		...
		add("guard",({npc}));
		if ( soldier[i] == "士兵一号" || soldier[i] == "士兵二号" || soldier[i] == "士兵三号" )
		{
		...
		else if ( soldier[i] == "士兵四号" || soldier[i] == "士兵五号" || soldier[i] == "士兵六号" )
		{
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 )
	{
		if ( ShoutTime == 0 )
		{
			SAY_CMD->say(me, sprintf("可恶！竟然牺牲了本将%d个部下！！我一定要替他们报仇雪恨！",hate)); 
			ShoutTime = 1;
		}
...
void win_bonus_callout( object me, object who )
{
	...
	log_file("boss.txt",short_time()+" 威武大将军被消灭\n");
	...
	CHAT_D->rumor_channel( 0, CHAT+"听说威武大将军已经被勇敢的"+who->get_name()+"所消灭！" ); 
...
{
	send_user( who, "%c%c%w%s", ':',3,5201,"威武大将军:\n    养兵千日，用兵一时。只要平日多作训练，真正到了战场才不会临阵大乱。\n"+
		sprintf(ESC"挑战\ntalk %x# fight\n",getoid(this_object())) +
		ESC"离开");
}
