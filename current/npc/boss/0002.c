...
void create()
{
	...
	set_skill(0423,40);
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"据说犬太郎又出没在海滨的东莱峡一带等待着勇士的挑战。" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" 犬太郎出现\n");
}
...
void win_bonus_callout( object me, object who )
{
	...
	log_file("boss.txt",short_time()+" 犬太郎被消灭\n");

        if( !objectp( who = me->get_enemy_4() ) ) return;

        if( owner = who->get_owner() ) who = owner;
        if( who->is_npc() ) return;
	CHAT_D->rumor_channel( 0, CHAT+"犬太郎已经被勇敢的玩家"+who->get_name()+"所消灭！" ); 
...
{
	send_user( who, "%c%c%w%s", ':',3,0352,"犬太郎:\n    怎么你们老爱找我麻烦！？\n"+
		sprintf(ESC"挑战\ntalk %x# fight\n",getoid(this_object())) +
		ESC"离开");
}
