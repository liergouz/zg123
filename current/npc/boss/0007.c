...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"有人看到驱尸道长出现在首阳山的夷齐墓，不知是真是假？" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" 驱尸道长出现\n");
}
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+"驱尸道长已经被勇敢的玩家所消灭" ); 
	log_file("boss.txt",short_time()+" 驱尸道长被消灭\n");
