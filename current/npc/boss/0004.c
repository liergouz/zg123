...
void heart_beat_walk()
{
	...
	{		
		...
		if ( gone_time(saytime) >=60 )
		{
			...
			if(random(100) < 40)
			{
				SAY_CMD->say(me, "我做的酒菜连大王都赞赏，你们也尝尝这人肉包子！！"); 
			}
		}
	}
	else if ( RelaxTime == 0 )
...
int perform_action( object who ) 
{
	...
	{
		pTime = time();
		SAY_CMD->say(me, "看看我这火，用这火烧出来的菜一定很美味！！");
...
void win_bonus_callout( object me, object who )
{
	...
	exp = 15000;
		...
		if( team[i]->get_legend(TASK_2_00, 4) && !team[i]->get_legend(TASK_2_00, 5) && team[i]->get_save("fb_002_001") < 1 )
		{
			...
			send_user(team[i],"%c%s",'!',sprintf(HIY"梁兴 %d/1",kk));		
		}
...
void stop_fight()
{
	...
	{
		SAY_CMD->say(this_object(), "哈哈。。。哈哈。。。今晚又有新鲜的肉吃了。？？嗯？我怎么会说又字？？");
		add_hp(2000);
	}
	::stop_fight();	
}