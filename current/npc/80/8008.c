...
void do_welcome2( object me, string arg )
{
	...
	case 1:
        ...
        case 3:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我晕我晕我晕晕晕，秦国也太牛了吧，居然能够让人变身为怪物？嘘，这可是顶级机密，前段时间听闻有大战，而跑到秦国去避难的百姓不知道受了什么蛊惑，有些接受了秦国的特殊训练，居然能够维持一段时间的怪物外形，又潜回我国九曲桥附近为秦国做内应，孰可忍，孰不可忍！一定要让他们知道叛徒的下场，一定要彻底摧毁秦国的这个阴谋！你看到那些像破罐子一样的东西了吗？是的，你去杀掉它们，你干掉10只罐中仙，我倒要看看，秦国还有什么招术！\n"ESC"接受任务\ntalk %x# welcome.4\n"ESC"离开", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_36,23) && !who->get_legend(TASK_36,24) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    不要管他们是不是齐国百姓，记住，叛国者死！\n"ESC"离开", me->get_name()));
        	else if ( who->get_legend(TASK_36,24) && !who->get_legend(TASK_36,25) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    叛徒死不足惜，我倒要看看，秦国还有什么招术！\n"ESC"完成任务\ntalk %x# welcome.5\n"ESC"离开", me->get_name(),getoid(me)));
        	break;
        case 4:
        	if ( who->get_level() >= 100 && !who->get_legend(TASK_36,23)  )
        	{
	...
	case 6:
		if ( who->get_level() >= 110 && !who->get_legend(TASK_44,13) )
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    现在天下七分，困守一地无异于故步自封，你得趁年轻到处看看，汲取各地军事文化的精髓，方能够高屋建瓴，得到更大的发展，光从军事领域来说，秦国无疑是现在最强的国家，而秦长城在抵御外虏的作用上是首屈一指的，若是抛除国籍的成见，驻守的蒙恬、王离将军对于避免国民遭受外虏侵袭上可是功不可没。如今秦赵交恶，我也不能给你更多指引，听闻秦总管善待有才之士，你去问问他秦长城的情况，应该他会乐于介绍，不至于难为你。\n"ESC"接受任务\ntalk %x# welcome.9\n"ESC"离开", me->get_name(),getoid(me)));
