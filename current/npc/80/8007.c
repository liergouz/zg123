...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    秦、赵大战于长平，我国与燕合力救赵。今赵军缺粮，派人到本王府，希望能给本国财政官通报一声，借点粮食，有助于赵军继续攻秦。本总管希望你速速将此事禀报本国财政官！\n"ESC"接受任务\ntalk %x# welcome.2\n"ESC"离开", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_09,19) && !who->get_legend(TASK_09,20) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    看来赵国气数已尽，怪不得别人，财政官不肯帮他，也是预料之中，削弱自身国力的事情谁都不愿去做！\n"ESC"离开", me->get_name()));
        	else if ( who->get_legend(TASK_09,26) && !who->get_legend(TASK_09,27) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    果然是英明的决策！出于仁义救助面临危亡的赵国，军威足以击退秦军，如果因为害怕削弱自身的国力，而不帮助赵国，这样为国家考虑，显然是错误的策略！\n"ESC"完成任务\ntalk %x# welcome.3\n"ESC"离开", me->get_name(),getoid(me)));
        	break;
        case 2:
        	if ( who->get_level() >= 35 && !who->get_legend(TASK_09,19)  )
        	{
	...
	case 8:
        ...
        case 11:
        	if ( who->get_level() >= 35 && !who->get_legend(TASK_37,29) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    听闻秦管家管事有方，诺大一个秦王府被他管理得头头是道，主子仆人上下井然，俗话说，没规矩不成方圆，我看我还得向秦管家讨教讨教，关于怎样管理好这大大小小的仆人，还真得下一番工夫。我看不如你替我去趟秦国，向秦管家取这管理的经。\n"ESC"接受任务\ntalk %x# welcome.12\n"ESC"离开", me->get_name(),getoid(me)));
        	break;
        case 12:
        	if ( who->get_level() >= 35 && !who->get_legend(TASK_37,29) )
        	{
        ...
        case 13:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    东海渔村一个月前突然百鸟汇集，原来竟然是天降祥瑞，凤凰临世，想不到几日前几个游手好闲的所谓大侠为了宝物竟然乘凤凰临产突施杀手，东海渔村突下天火，凤凰浴火而重生，也烧死了那几个贪心的家伙，我国旅秦侨民马显报告说卜卦得知凤凰涅磐留下一块凤凰玉佩，传言有青春永驻的功效，王后闻说甚为挂念，命老夫觅人秘密寻找，本官见你聪明伶俐，又多次圆满完成交代你的任务，决定将这项美差交与你，如何，你可能胜任？\n"ESC"接受任务\ntalk %x# welcome.14\n"ESC"离开", me->get_name(),getoid(me)));
