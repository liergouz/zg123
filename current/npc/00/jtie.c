...
void do_look( object who )
{
	...
	{
		send_user( who, "%c%s", '!', sprintf("您的采矿技能不足%d级！", SKILL_LEVEL));
		return;
	}
	if (who->get_skill_degree(THIS_SKILL)<SKILL_EXP) 
	{
		send_user( who, "%c%s", '!', sprintf("您的采矿熟练度不足%d！", SKILL_EXP));
		return;
	}
	...
	if (!item) 
	{
		send_user( who, "%c%s", '!', sprintf("您没有%d级以上的采矿工具！", TOOL_LEVEL));
		return;
	}
	...
	send_user(who, "%c%c%w%s", 0x5a, 0, 6, "挖矿中……");	
...
void into_effect( object who, string arg )
{
	...
	if (item)
	{
		...
		printf( ECHO "您获得一块%s。", item->get_name() );
