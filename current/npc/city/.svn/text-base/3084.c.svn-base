...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,5,"生产工具" );
			send_user( who, "%c%s", '!', "得到任务 "HIY"生产工具");
        	}
        	break;       
        case 3:
        	if ( who->get_legend(TASK_15,25) && !who->get_legend(TASK_15,26) )
        	{
        		...
        		if ( !stringp(tmp) )
        			return ;
        		if (TASK_LEGEND_D->check_task_item(who,"锄头",1) != 1 )
        			return;
			if (TASK_LEGEND_D->check_task_item(who,"镰刀",1) != 1 )
			{
				...
				item->add_to_user(p);
     			...
     			if (TASK_LEGEND_D->check_task_item(who,"簸箕",1) != 1 )
        		{
				item = new("item/98/0068");
				p = item->move(who,-1);
				item->add_to_user(p);
			...
			who->add_cash(5600);
			send_user(who,"%c%s",';',sprintf("生产工具 经验 6500 潜能 265 金钱 5600 %s 1",tmp->get_name()));
	...
	case 37:
		if ( who->get_legend(TASK_42,10) && !who->get_legend(TASK_42,11) )
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    难道上次你完成任务只是侥幸?那算了，别找我了，我乐得休息休息。\n"ESC"离开", me->get_name()));
