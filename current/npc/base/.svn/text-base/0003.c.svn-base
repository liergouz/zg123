...
//		if ( who->get_legend(TASK_NEWBIE_01, 8) && !who->get_legend(TASK_NEWBIE_01, 9))
		if ( who->get_legend(TASK_48,9) && !who->get_legend(TASK_48, 10))
			send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s:\n    前几天就有个奇怪的人告诉我，过几天会有个叫%s的人拿着我丢失的家族玉佩来找我，果然来了，就是你吧！真是太好了，我得好好的谢谢你！\n"ESC"完成任务\ntalk %x# welcome.2\n",me->get_name(),who->get_name(),getoid(me)));
...
//		if ( !who->get_legend(TASK_NEWBIE_01, 8) || who->get_legend(TASK_NEWBIE_01, 9))
//			return ;
//		if ( who->get_legend(TASK_48,9) && objectp( item = present("家族玉佩", who, 1, MAX_CARRY) ))
		if ( who->get_legend(TASK_48,9) && !who->get_legend(TASK_48, 10))
		{
...
//			item->set_amount(10);
			...
			who->add_cash(500);
			send_user(who,"%c%s",';',"家族玉佩 经验 260 潜能 30 金钱 500 新手项链 1");
...
//		if ( !who->get_legend(TASK_NEWBIE_01, 10) )
		if ( who->get_legend(TASK_48, 10) )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,105,"探望病人" );
			send_user( who, "%c%s", '!', "你接受了"HIY"探望病人"NOR"任务");
		}			
		break;
	case 5:
		if( who->get_legend(TASK_40, 24) && !who->get_legend(TASK_40, 25) )
