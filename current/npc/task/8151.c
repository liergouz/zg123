...
void create()
{
        set_name( "窃贼" + NPC_NAME_D->get_thief_name() );
...
void win_bonus_callout( object me, object who )
{
	...
	item = new( "/item/98/9870" );        
        ...
        write_user( who, ECHO + me->get_name() + "叫道：“大侠放过我吧，我再也不敢偷东西了！”\n"
                ECHO "您大义凛然地教训道：“回去好好做人！”，说罢抬手将他放走。" );

        send_user( who, "%c%s", '!', sprintf( "把" HIY " %s " NOR "交还王捕头。", item->get_name() ) );
	...
	who->set_quest("thief.bonus",4);
        ...
        {
		cTmp = sprintf("您得到" HIY "%s" NOR "！",item->get_name());
		x =item->move(who, -1);
		item->add_to_user(x);
        ...
        {
        	cTmp = sprintf("" HIY "%s" NOR "掉在地上！",item->get_name());
