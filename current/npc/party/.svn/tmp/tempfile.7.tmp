...
//	}	
	...
	if (!me->get("busy"))
	{
	        if( !tId || ( size = sizeof(team) ) < 3 )
	        {
	                send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), me->get_name() + "：\n    尔等寥寥鼠辈也敢来挑战本大爷吗？还不去多叫几个人来！（活动须以队伍形式存在，队伍人数≥3人）\n"ESC"离开" );
	                return;
	        }
	        if(  !who->is_leader() )
	        {
		...
		for (i=0;i<size;i++)		
		{
			...
			{
	                	send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), me->get_name() + "：\n    你的手下是不是没到齐啊？怎么还有几个我看不到呢？（队伍成员须全在NPC视线范围之内）\n"ESC"离开" );				
				return;
			}
		}
		if (who->is_leader() && size>=3)
		{
