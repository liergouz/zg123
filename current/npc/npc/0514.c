...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_11, 16) && !who->get_legend(TASK_11, 17) && random(100) < 30)
	{
		...
		if ( (i=who->add_save_2("wtjshitan",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,1,"无奈的求助（2）(完成)" );
		}
	...
	if( who->get_legend(TASK_13, 25) && !who->get_legend(TASK_13, 26) && random(100) < 27)
	{
		...
		if ( (i=who->add_save_2("swqizhi",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,21,"棋子(完成)" );
		}
	...
	{
		if ( objectp( item = present("红宝石", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
			if ( item->get_amount() < 10 )
				item->add_amount(1);
		}
		...
		if ( (i=who->add_save_2("zghongbaoshi",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,24,"珍贵的红宝石(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,24);
		send_user(who,"%c%s",'!',sprintf(HIY"红宝石 %d/10",i));		
	}
	if( who->get_legend(TASK_14, 15) && !who->get_legend(TASK_14, 16) && random(100) < 15)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,28,"二胡(完成)" );
	...
	{
		if ( objectp( item = present("黄连", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("frhuanglian",1)) >= 5 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,32,"夫人的怪病（3）(完成)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,32);
		send_user(who,"%c%s",'!',sprintf(HIY"黄连 %d/5",i));		
	}
}
// 函数：掉宝奖励
