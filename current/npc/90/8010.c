...
//		result = ESC"老村长的委托。\n" + sprintf("talk %x# task.1\n", id); 	
	...
	if ( who->get_legend(TASK_49, 7) && !who->get_legend(TASK_49, 8))
        ...
        if( who->get_fam_name() == "" )
                ...
                send_user( who, "%c%c%w%s", ':', 3, 4153, get_name() + "：\n这位" + NPC_RANK_D->get_respect(who) + "如果想去高人隐居的名山秀地，我这里是不收费的。\n"
                        ESC "去桃花源。\n"   + sprintf( "talk %x# 1\n", id ) +
                        ESC "去蜀山。\n"     + sprintf( "talk %x# 2\n", id ) +
                        ESC "去大周军营。\n" + sprintf( "talk %x# 3\n", id ) +
                        ESC "去庄院。\n"     + sprintf( "talk %x# 4\n", id ) +
...
/*      if( present("钱票", who, 1, MAX_CARRY) )
        ...
        case "桃花源" : flag = 1;  break;
          case "蜀山" : flag = 2;  break;
        case "禁卫军" : flag = 3;  break;
          case "唐门" : flag = 4;  break;
          case "茅山" : flag = 5;  break;
        case "昆仑山" : flag = 6;  break;
        case "云梦泽" : flag = 7;  break;
        }
